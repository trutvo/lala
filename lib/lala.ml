...
let add args =
  List.map float_of_string args |> List.fold_left (+.) 0. |> string_of_float

let sub args =
  let r = match List.map float_of_string args with
  | [] -> 0.
  | [n] -> n *. -1.
...
let mul args =
  List.map float_of_string args |> List.fold_left ( *. ) 1. |> string_of_float
...
let eval_code code = 
  ...
  eval { operations = default_ops_map } "" sexp
